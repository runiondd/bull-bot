"""
Long call strategy — buy a directional ITM/ATM call for bullish exposure.
No IV rank gate; suitable when directional conviction is high.

Parameters:
  - dte: target days-to-expiry
  - delta: target delta for the long call (e.g., 0.60 = moderately ITM)
"""
...
class LongCall(Strategy):
    ...
    def evaluate(
        ...
        best = _pick_by_delta(expiry_calls, snapshot.spot, t_years, False, target_delta)
        if best is None:
            return None

        option_symbol = _make_osi(snapshot.ticker, chosen_expiry, best.strike, "C")
        ...
        return Signal(
            ...
            rationale=(
                f"Long {best.strike}C {chosen_expiry} "
